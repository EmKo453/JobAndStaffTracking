...
def update_metrics(n_intervals, location_value, graph_dropdown, input1):
    ...
    total_employees_graph = go.Figure(go.Scatter(x = data['Time'], y = data['Number of Employees'], name=setup.Location_name, mode='lines'))
    ...
    for i in locations_graph:
        ...
        location_employees_graph.add_trace(go.Scatter(x = data['Time'], y = data['Number of Employees'], name=i, mode='lines'))
        if (data['Number of Employees']):
